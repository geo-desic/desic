...
{
    public static async Task<StringHttpResponse> SendAsyncAndReadResponseAsString(this HttpClient httpClient, FluentHttpRequest request, ITestOutputHelper? output = null)
    {
        output?.WriteLine($"Attempting http request: [{request.HttpRequestMessage.Method}] {GetFullUrl(baseAddress: httpClient.BaseAddress, requestUri: request.HttpRequestMessage.RequestUri)}");
    ...
    public static async Task<ObjectHttpResponse<T>> SendAsyncAndReadResponseAsJson<T>(this HttpClient httpClient, FluentHttpRequest request, ITestOutputHelper? output = null)
    {
        output?.WriteLine($"Attempting http request: [{request.HttpRequestMessage.Method}] {GetFullUrl(baseAddress: httpClient.BaseAddress, requestUri: request.HttpRequestMessage.RequestUri)}");
        var response = await httpClient.SendAsync(request.HttpRequestMessage, TestContext.Current.CancellationToken);

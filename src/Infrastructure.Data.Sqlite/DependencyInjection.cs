...
public static class DependencyInjection
{
    public static IServiceCollection AddSqliteInfrastructure(this IServiceCollection services, IConfiguration config, string? connectionString = null)
        => services
            ...
            .ConfigureApplicationDbContextForSqlite(connectionString: connectionString ?? config.GetValue("connection", config.GetConnectionString("Sqlite")), setMigrationsAssembly: config.GetValue("Databases:Desic:Migrations:Enabled", true), useSeeding: config.GetValue("Databases:Desic:Seeding:Enabled", false));
}

...
{
    [Description("The requested count of items to be returned that is not guaranteed to be honored if it is too large")]
    [DefaultValue(ListRequests.DefaultCount)]
    [Range(minimum: 0, maximum: int.MaxValue)]
    public int Count { get; set; } = ListRequests.DefaultCount;

    [Description("The requested inclusion or omiitance of the total count of items matching the query after applying any filters but before pagination that is not guaranteed to be honored")]

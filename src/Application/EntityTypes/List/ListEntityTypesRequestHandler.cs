...
public class ListEntityTypesRequestHandler(ILogger<ListEntityTypesRequestHandler> logger, IApplicationDbContext dbContext) : IRequestHandler<ListEntityTypesRequest, Result<ListResult<EntityType>>>
{
    ...
    public async Task<Result<ListResult<EntityType>>> Handle(ListEntityTypesRequest request, CancellationToken cancellationToken)
    {
        ...
        return await query.ToListResultAsync(startIndex: request.StartIndex, takeCount: request.Count, includeTotalCount: IncludeTotalCount, cancellationToken: cancellationToken);
    }
}

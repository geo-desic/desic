...
public sealed class InitializeApplicationDatabaseRequestHandler(IConfiguration config, IOptions<InitializeApplicationDatabaseOptions> options, ILogger<InitializeApplicationDatabaseRequestHandler> logger) : IRequestHandler<InitializeApplicationDatabaseRequest, Task>
{
    ...
    private readonly ILogger<InitializeApplicationDatabaseRequestHandler> _logger = logger ?? NullLogger<InitializeApplicationDatabaseRequestHandler>.Instance;
    ...
    public async Task Handle(InitializeApplicationDatabaseRequest request, CancellationToken cancellationToken)
    {
        ...
        _databaseName = request.DatabaseName ?? _options.Name ?? throw new InvalidOperationException("Database name is not specified in options or method parameters");

        using var connection = await GetConnection(request.ConnectionString, cancellationToken);

...
- Data **format** for domain entities (e.g. [User](Users/User.cs))
  ...
  - Note that this data format in many cases will not be the same as what is returned from the API. The data can be reshaped (both incoming and outgoing) by data transfer objects which the API will utilize.
- Abstraction interfaces
- Exception classes
- Enums (e.g. [SystemEntityType](EntityTypes/SystemEntityType.cs))

This library is not for **handling** requests to get or persist data. Those handlers should be in the application or infrastructure projects.
